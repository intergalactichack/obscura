...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        while True:
            ...
            # addition for hackathon
            rawtextblocks.append(decoded_text)
            rawtextblockcenters.append(int(pos[0][0]))


        # addition for hackathon
        if rawtextblocks is not None and len(rawtextblocks) == 2: 
            payload =  dict()
            measurementstr = "-"
            measurement = -1.0
            try:
                if rawtextblockcenters[0] < rawtextblockcenters[1]:
                    measurementstr = rawtextblocks[0]+"."+rawtextblocks[1]
                else: 
                    measurementstr = rawtextblocks[1]+"."+rawtextblocks[0]

                measurement = float(measurementstr)
                ...
                print("error: could not prepare payload   - " + rawtextblocks[0]+ " . " + rawtextblocks[1])

        else: 
            pass      
            # print("rejecting recognized text blocks as not repeated ~ instable recognition and/or wrong block count")
